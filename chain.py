...
class Blockchain(object):
    ...
    def valid_proof(lastProof, proof):
        ...
        # 验证该字符前n位是否为0，如果符合要求，就返回True，否则 就返回False
        return guessHash[:difficulty] == ('0' * difficulty)

    def get_max_balance(self):
        balance_dict = {'0': 32767.0}
        for block in self.chain:
            for transaction in block['transactions']:
                sender = transaction['sender']
                recipient = transaction['recipient']
                amount = transaction['amount']
                balance_old = float(balance_dict[sender]) if sender in balance_dict.keys() else 0.0
                balance_dict[sender] = balance_old - float(amount)
                balance_old = float(balance_dict[recipient]) if recipient in balance_dict.keys() else 0.0
                balance_dict[recipient] = balance_old + float(amount)
        return balance_dict
